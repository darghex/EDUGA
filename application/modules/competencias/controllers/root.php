...
class Root extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->variables=array('modulo'=>'competencias','id'=>'id_competencias','modelo'=>'model_competencias');
	...
	public function index($id_cursos)
	{
		$this->lista($id_cursos);
	}


	public function lista($id_cursos)
	{
	...
	public function guardar()
	{
		...
		$this->form_validation->set_rules('id_cursos', 'Curso', 'required|xss_clean');
  ...
  #echo validation_errors(); exit;
			...
			$accion_url=base_url().$this->uri->segment(1).'/'.$this->uri->segment(2).'/index/'.$this->input->post ('id_cursos').'/'.$id.'/guardado_ok';
			redirect($accion_url);
