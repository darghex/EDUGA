       ...
       <div class="evaluacion_preg">
        ...
        <form action="">
          <?php foreach (json_decode($value->variables_pregunta) as $subkey => $subvalue): ?>
            <input <?php if ($value->respuesta==$subvalue): ?> checked <?php endif ?> class="radio" type="radio" id="p<?php echo $value->id_encuestas_detalle; ?>" name="p<?php echo $value->id_encuestas_detalle; ?>" value="<?php echo $subvalue; ?>"><span><?php echo $subvalue; ?></span>
          <?php endforeach ?>   
          <?php $arr_res[]=$value->id_encuestas_respuestas; ?>
        </form>                                
      </div>
    <?php endif ?>
     ...
     <div class="evaluacion_preg">
      ...
      <form action="">
       <select class="select" name="p<?php echo $value->id_encuestas_detalle; ?>" id="p<?php echo $value->id_encuestas_detalle; ?>">
        <?php foreach (json_decode($value->variables_pregunta) as $subkey => $subvalue): ?> 
          <option <?php if ($value->respuesta==$subvalue): ?> selected <?php endif ?> value="<?php echo $subvalue; ?>"><?php echo $subvalue; ?></option>
        <?php endforeach ?>    
      </select>
      <?php $arr_res[]=$value->id_encuestas_respuestas; ?>
    </form>                                
...
<?php if ($value->tipo_pregunta==3): ?>
  <div class="evaluacion_preg">
    <p><?php echo $value->pregunta; ?></p>
    <form action="">
     <textarea class="text" name="p<?php echo $value->id_encuestas_detalle; ?>" id="p<?php echo $value->id_encuestas_detalle; ?>" cols="30" rows="10" placeholder=""><?php echo $value->respuesta; ?></textarea>
     <?php $arr_res[]=$value->id_encuestas_respuestas; ?>
   </form>                                
 </div>
<?php endif ?>
...
<form action="">
  ...
  <input type="hidden" id="rtas" name="rtas" value="<?php echo implode(",", $arr_res); ?>">
        ...
        jQuery(document).ready(function($) {
          ...
          jQuery('#send_encuesta').click(function(event) {
            ...
            if (enviar==true)  {
              ...
              jQuery.ajax({
                url:'<?php echo base_url(); ?>encuestas/publico/validar_encuesta_editar/<?php echo $this->uri->segment(3); ?>',
                type: "post",
                data:({
                  data:data
                }),
